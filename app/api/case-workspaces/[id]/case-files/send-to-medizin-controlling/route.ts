...
import { bulkAdvanceToMedizinControlling } from "@/lib/services/case-file-service";
...
export async function POST(
  ...
  try {
    ...
    const items = await bulkAdvanceToMedizinControlling(id, caseFileIds, access.user!.id);

    await logAuditEvent({
      actorUserId: access.user!.id,
      action: "case_files_advanced_to_medizin_controlling",
