...
import { bulkReturnToProcessing } from "@/lib/services/case-file-service";
...
export async function POST(
  ...
  try {
    ...
    const items = await bulkReturnToProcessing(id, caseFileIds, access.user!.id);

    await logAuditEvent({
      actorUserId: access.user!.id,
      action: "case_files_returned_to_processing",
